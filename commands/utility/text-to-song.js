...
/**
 * Processes the text-to-song generation.
 * @param {Object} interaction - The Discord interaction object.
...
async function processSongGeneration(interaction, initialReply, images, songDetails, supabaseToken, userId, createdUser, startTimestamp, privateSong = false) {
    ...
    try {
        ...
        const generationId = await createSongGeneration(
            ...
            interaction.guildId, interaction.channelId, interaction.options.getString("song_description"), 
        ...
        try {
            const firstReply = `${songDetails.firstReplyMessage}\n${GENERAL_MESSAGES.SONG_PROMPT_NOTICE}${inlineCode(interation.option.getString("song_description"))}\n`;
        ...
        if (!privateSong) {
            ...
            await postToPrivateCreateSongs(interaction, outputMessage, songFileAttachment, false, "song", "text-to-song");
        }
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName(COMMAND_NAMES.TEXT_TO_SONG)
        .setDescription(GENERAL_MESSAGES.SONG_FROM_TEXT)
        .setDMPermission(false)
        .addStringOption(option => 
            option.setName('song_description')
                .setDescription(PARAM_DESCRIPTIONS.SONG_PROMPT)
                .setRequired(true)
                .setMaxLength(PARAMS.TEXT_TO_SONG_LENGTH))
        .addStringOption(option =>
        ...
        .addAttachmentOption(option => 
            option.setName('image1').setDescription(PARAM_DESCRIPTIONS.IMAGE_TO_SONG_FIRST_IMAGE).setRequired(false))
        .addAttachmentOption(option => 
    ...
    async execute(interaction) {
        ...
        if ((PARAMS.TEXT_TO_SONG_IN_MAINTENANCE || process.env.PAUSE_ALL_COMMANDS.toLowerCase() == "true") && 
             !interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
            await interaction.reply({ content: `${commandCaller} ${LOG_ERRORS.TEXT_TO_SONG_IN_MAINTENANCE}`, ephemeral: true });
            throw Error(LOG_ERRORS.TEXT_TO_SONG_IN_MAINTENANCE);
        }
        ...
        const initialReply = await interaction.reply({
            content: `${commandCaller} ${GENERAL_MESSAGES.INITIAL_TEXT_TO_SONG_REPLY}`,
        ...
        const validatedSongDescription = await validateDescription(interaction.options.getString('song_description'));
        if (validatedSongDescription.error) {
            await handleError(interaction, initialReply, validatedSongDescription.error, null, null, ephemerality);
            throw Error(validatedSongDescription.error);
        }
        ...
        const songDetails = getSongDetails(interaction, images.valid.length, true, true);
        try {
            await interaction.editReply({
            ...
            console.log("COMMAND: Could not update the initial reply with the images the user sent and with the song vibe/genres.")
        }
